...
sd2 <- cbind(sd, c(mean(sqrt(2*unlist(tNULLr.2$sds)^2/12)), mean(sqrt(2*unlist(t60r.2$sds)^2/12)), mean(sqrt(2*unlist(t50r.2$sds)^2/12)), mean(sqrt(2*unlist(t40r.2$sds)^2/12)), mean(sqrt(2*unlist(t30r.2$sds)^2/12))))
colnames(sd2)[10] <- "True"
colnames(sd2)[11] <- "Average"
@

<<tpfTable, echo = FALSE, results = 'asis'>>=
tbl <- data.frame(Bias.1 = bias1, Est.SD.1 = sd[11], True.SD.1 = sd[10], Bias.4 = bias4, Est.SD.4 = sd2[11], True.SD.4 = sd[10], row.names = c("RR = 1", "RR = 0.6", "RR = 0.5", "RR = 0.4", "RR = 0.3"))
xtable(tbl, align = c("cc|ccc|ccc"), caption = "The bias for the test-positive fraction quadratic estimates of the Relative Risk and the standard deviation of the difference in arm-specific averages (T) from 10,000 unconstrained intervention allocations.")
@
